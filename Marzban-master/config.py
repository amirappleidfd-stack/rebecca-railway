...
# Railway provides a single $PORT for the web service. Bind to it unless an
# explicit UVICORN_PORT is supplied. Never hardcode 8000.
UVICORN_PORT = int(os.environ.get("PORT") or config("UVICORN_PORT", default=8000))
...
def _resolve_database_url() -> str:
    ...
    Priority:
      ...
      2. Railway Postgres plugin's DATABASE_URL / POSTGRES_URL (preferred on Railway)
      3. SQLite fallback (default for local / quick deploys)
    """
    explicit = config("SQLALCHEMY_DATABASE_URL", default="")
    if explicit:
        return explicit

    # Railway PostgreSQL plugin exposes DATABASE_URL (and POSTGRES_URL).
    pg = os.environ.get("DATABASE_URL") or os.environ.get("POSTGRES_URL")
    if pg:
        # SQLAlchemy needs the postgresql+psycopg2:// (or +asyncpg) dialect.
        if pg.startswith("postgres://"):
            pg = pg.replace("postgres://", "postgresql://", 1)
        if pg.startswith("postgresql://"):
            pg = pg.replace("postgresql://", "postgresql+psycopg2://", 1)
        return pg
