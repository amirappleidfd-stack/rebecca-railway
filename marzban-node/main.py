...
from config import (SERVICE_HOST, SERVICE_PORT, SERVICE_PROTOCOL,
                    SERVICE_TLS, SSL_CERT_FILE, SSL_KEY_FILE,
...
if __name__ == "__main__":
    ...
    if SERVICE_PROTOCOL == 'rpyc':
        ...
        if SERVICE_TLS:
            # Mutual-TLS mode. SSL_CLIENT_CERT_FILE is required.
            if not SSL_CLIENT_CERT_FILE:
                logger.error("SSL_CLIENT_CERT_FILE is required for rest service with TLS enabled.")
                exit(0)

            logger.info(f"Node service (REST + TLS) running on :{SERVICE_PORT}")
            uvicorn.run(
                rest_service.app,
                host=SERVICE_HOST,
                port=SERVICE_PORT,
            ...
            # Plain HTTP mode. Railway terminates TLS at the edge and reaches
            # this node over the private network, so no in-container TLS is
            # needed (and a client cert cannot be supplied by the platform).
            logger.info(f"Node service (REST) running on {SERVICE_HOST}:{SERVICE_PORT}")
            uvicorn.run(
                rest_service.app,
                host=SERVICE_HOST,
                port=SERVICE_PORT
